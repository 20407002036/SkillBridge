"""
Groq AI Service for SkillBridge
Handles all Groq API interactions for resume analysis and roadmap generation
Primary LLM provider with Gemini as fallback
"""
import os
import json
import time
...
class GroqService:
    """
    Main Groq service class for AI operations
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        # Create custom httpx client without proxy to avoid proxy-related issues
        http_client = httpx.Client(
            timeout=httpx.Timeout(60.0, connect=10.0)
        )

        # Initialize Groq client with custom http_client
        self.client = Groq(
    ...
    def analyze_resume(self, resume_text: str, target_skill: str) -> Tuple[Dict, List[Dict]]:
        """
        Analyze resume using Groq and extract user profile + recommend skills
...
8. IF THE resume_text DON'T RESEMBLE A RESUME, STOP
...
"""
            ...
            try:
                chat_completion = self.client.chat.completions.create(
                    messages=[
                        {
                            "role": "system",
                            "content": "You are an expert AI assistant that responds only with valid JSON. Do not include any markdown formatting or additional text."
                        },
                        {
                            "role": "user",
                            "content": prompt
                ...
                if attempt < self.max_retries - 1:
                    print(f"Groq API attempt {attempt + 1} failed: {e}. Retrying in {self.retry_delay}s...")
    ...
    def _parse_resume_analysis_response(self, response: str, target_skill: str) -> Tuple[Dict, List[Dict]]:
        """Parse Groq response for resume analysis"""
        try:
            ...
            print(f"Failed to parse Groq resume analysis response: {e}")
    ...
    def _parse_roadmap_response(self, response: str, preferences: Dict, user_profile: Dict) -> Dict:
        """Parse Groq response for roadmap generation"""
        try:
            # Clean response (remove any markdown formatting)
            cleaned_response = response.strip()
            if cleaned_response.startswith('```json'):
            ...
            print(f"Failed to parse Groq roadmap response: {e}")
            return self._fallback_roadmap_generation([], preferences, user_profile)

    def _fallback_resume_analysis(self, resume_text: str, target_skill: str) -> Tuple[Dict, List[Dict]]:
        """Fallback resume analysis using Gemini if Groq fails"""
        try:
            from ..gemini import GeminiService
            gemini_service = GeminiService()
            print("Falling back to Gemini for resume analysis...")
            return gemini_service.analyze_resume(resume_text, target_skill)
        except Exception as e:
    ...
    def _fallback_roadmap_generation(self, selected_skills: List[str], preferences: Dict, user_profile: Dict) -> Dict:
        """Fallback roadmap generation using Gemini if Groq fails"""
        try:
            from ..gemini import GeminiService
            gemini_service = GeminiService()
            print("Falling back to Gemini for roadmap generation...")
            return gemini_service.generate_roadmap(selected_skills, preferences, user_profile)
        except Exception as e:
            print(f"Gemini fallback also failed: {e}")
            # Return mock data as last resort
            return {
                ...
                "notes": "This is a basic roadmap. For personalized recommendations, ensure Groq or Gemini API is working properly."
            }
